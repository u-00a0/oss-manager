...
impl TransferManager {
    ...
    pub async fn upload(
        ...
        if local_path.is_file() {
            ...
            if !recursive {
                return Err(anyhow!(
                    "Source is a directory, use -r to upload recursively"
                ));
            }
            ...
            for entry in walker.into_iter().filter_map(|e| e.ok()) {
                ...
                if entry.file_type().is_file() {
                    ...
                    let sub_cb = if let Some(main_cb) = &progress_callback {
                        let acc =
                            transferred_accumulator.load(std::sync::atomic::Ordering::Relaxed);
        ...
        if recursive {
            ...
            for obj in objects {
                if let Some(key) = obj.key {
                    ...
                    info!(
                        "Copying (Cloud): s3://{}/{} -> s3://{}/{}",
                        src_bucket, key, dest_bucket, final_key
                    );
                    self.ops
                        .copy_object(src_bucket, &key, dest_bucket, &final_key)
                        .await?;
                }
            }
        } else {
    ...
    pub async fn move_cloud(
        ...
        if is_batch {
            self.copy_cloud(src_bucket, src_key, dest_bucket, dest_key, true)
                .await?;
            let keys: Vec<String> = objects.into_iter().filter_map(|o| o.key).collect();
            self.ops.delete_objects(src_bucket, keys).await?;
        } else {
            self.copy_cloud(src_bucket, src_key, dest_bucket, dest_key, false)
                .await?;
            ...
            if objects.is_empty() {
                ...
                if !key.ends_with('/') {
                    ...
                    if !objects_retry.is_empty() {
                        let keys: Vec<String> =
                            objects_retry.into_iter().filter_map(|o| o.key).collect();
